...
class DealFile
  @@tableName = "`crack_36wmodel`"
  @@tableNameStr = "crack_36wmodel"
  @@itemIdBegin = $inputParam2

  def self.getInsertSta(itemHash)

    colListStr = "( `id`,`user_name`,`encrypt_pwd`,`email`,`regip`,`regdate`,`salt`,`secques`) "
  ...
  def self.Deal(inputFileName)
    ...
    lineCatch = 0;
    ticks = 0;
    outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"begin insert 36wmodel source id "+@@itemIdBegin.to_s + ", 1 tick means 100,000"))
...
#    begin
    while(1)
      ...
      itemSt["uid"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["username"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["password"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["email"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["myid"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["myidkey"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["regip"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["regdate"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["lastloginip"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["lastlogintime"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["salt"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      itemSt["secques"] = DealFile.FormatStr(ei.shift.text,outSpcFile)
      if itemSt["uid"] == "uid"
        next
      end
      ...
      if(i>100000)
        i=0
        ticks=ticks+1
        outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"next insert 36wmodel source id "+@@itemIdBegin.to_s + ", 1 tick means 100,000"))
        outSqlFile.close
  ...
  #  rescue => err
   ...
   # ensure
      ...
      outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"end insert 36wmodel source id "+@@itemIdBegin.to_s + ", 1 tick means 100,000"))
