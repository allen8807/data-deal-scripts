...
#end
#$inputParam = "test"
$inputParam = "qqdatabases.log.gbk.utf8"
$inputParam2 = 1
...
#  end
  ...
  def self.Split(org_str)
    first= org_str.index("----")
    second = org_str.reverse.index("----")
    strS =[""]
    if(first!=nil && second!=nil)
      if(first<org_str.length - second -4)
        strS[0] = org_str[0,first]
        strS[1] = org_str[first+4,org_str.length-first-second-8]
        strS[2] = org_str[0-second,second]
      end
    end
...
#如果含有特殊字符取出来
    ...
    lineCatch = 0;
    ticks = 0;
    outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"begin insert qq source id "+@@itemIdBegin.to_s + ", 1 tick about 100,000"))
    begin
      while line = inFile.gets
        ...
        if(i>100000)
          ticks=ticks+1
          i=0
          outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"next insert qq source id "+@@itemIdBegin.to_s + ", 1 tick about 100,000"))
          outSqlFile.close
        ...
        if(lines.length != @@fieldNum)
          ...
          if(line.index("\"")!=nil || line.index("\\")!=nil)
            ...
            #  line.gsub!(/'/,"\\\'")
            lines = DealFile.Split(line)
            spcSqlSt ="INSERT INTO "+@@tableName+" (`id`,`qq`,`plain_pwd`,`status`) VALUES ("+@@itemIdBegin.to_s+
                ", \""+lines[0].to_s+"\" , \""+lines[1].to_s+"\" , \""+lines[2].to_s+"\" );"
            @@itemIdBegin=@@itemIdBegin+1
            outSqlFile.puts(spcSqlSt)
            outSpcSqlFile.puts(spcSqlSt)
          else
            lineSuc = lineSuc+1;
            sqlSt ="INSERT INTO "+@@tableName+" (`id`,`qq`,`plain_pwd`,`status`) VALUES ("+@@itemIdBegin.to_s+
                ", \""+lines[0].to_s+"\" , \""+lines[1].to_s+"\" , \""+lines[2].to_s+"\" );"
            @@itemIdBegin=@@itemIdBegin+1
      ...
      outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"end insert qq source id "+@@itemIdBegin.to_s + ", 1 tick about 100,000"))
