...
#  inputParam2 = 1
#end
$inputParam = "total.catch"
$inputParam2 = 13635762
...
class DealFile
  @@tableName = "`crack_duduniu`"
  @@tableNameStr = "crack_duduniu"
...
#如果含有特殊字符取出来
    ...
    lineCatch = 0;
    ticks = 0;
    outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"begin insert duduniu source id "+@@itemIdBegin.to_s + ", 1 tick means 100,000"))
    begin
      while line = inFile.gets
        ...
        if(i>100000)
          i=0
          ticks=ticks+1
          outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"next insert duduniu source id "+@@itemIdBegin.to_s + ", 1 tick means 100,000"))
          outSqlFile.close
      ...
      outSqlFile.puts(Log.commit(ticks,@@tableNameStr,"end insert duduniu source id "+@@itemIdBegin.to_s + ", 1 tick means 100,000"))
